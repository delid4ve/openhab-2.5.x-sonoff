...
public class SonoffAccountHandler extends BaseBridgeHandler
    ...
    public void initialize() {
        ...
        String email = (String) getConfig().get("email");
        String password = (String) getConfig().get("password");
        String mode = (String) getConfig().get("accessmode");
        this.mode = mode;
        logger.info("Sonoff mode set to: {}", mode);

        commandManager.start(mode);
        restoreStates();
        queueTask = scheduler.scheduleWithFixedDelay(commandManager, 0, 100, TimeUnit.MILLISECONDS);

        connectionManager.start(email, password, mode);
        createTasks();
    }
